...
class ExternalPathPermissionCheck(PermissionCheck):
    ...
    @override
    def is_required(self, args: dict[str, Any]) -> bool:
        """Check if a given path is outside all allowed directories."""
        path = args.get("path")
        if not path:
            return False
        ...
        try:
            ...
            for allowed_dir in self.allowed_dirs:
                try:
                    ...
                    continue  # Check next allowed directory
            return True  # Path is outside all allowed directories
        except Exception:
            return False  # On error, don't require permission
...
# Values can be booleans or callables that take tool arguments and return bool.
